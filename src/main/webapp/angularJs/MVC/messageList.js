/**
 * declare an angularJs directive "message.list" in module "directives" , type : element usage:
...
(function (angular) {
    ...
    function _controller($scope, messageService) {
        ...
        $scope.openMsg = function (Id) {
            console.log("opening msg " + Id);
            $scope.service.openMsg(Id);
        };
    }
    
    var rootDir = _getCurrentPath();
    
    console.log("init messageList...");
    
    var myDirective = {
        restrict: 'E',       
        templateUrl: rootDir + 'messageList.html',
    ...
    angular.module('directives').directive('message.list', function () {
        return myDirective;
    });
